...
public class SwipeToLoadLayout extends ViewGroup {
	...
	/**
	 * <b>ATTRIBUTE:</b> a switcher indicate whither refresh function is enabled
	 */
	private boolean mRefreshEnabled = true;

	/**
	 * <b>ATTRIBUTE:</b> a switcher indicate whiter load more function is
	 * enabled
	 */
	private boolean mLoadMoreEnabled = true;

	/**
	 * <b>ATTRIBUTE:</b> the style default classic
	 */
	private int mStyle = STYLE.CLASSIC;

	/**
	 * <b>ATTRIBUTE:</b> offset to trigger refresh
	 */
	private float mRefreshTriggerOffset;

	/**
	 * <b>ATTRIBUTE:</b> offset to trigger load more
	 */
	private float mLoadMoreTriggerOffset;

	/**
	 * <b>ATTRIBUTE:</b> the max value of top offset
	 */
	private float mRefreshFinalDragOffset;

	/**
	 * <b>ATTRIBUTE:</b> the max value of bottom offset
	 */
	private float mLoadMoreFinalDragOffset;

	/**
	 * <b>ATTRIBUTE:</b> Scrolling duration swiping to refresh -> default
	 */
	private int mSwipingToRefreshToDefaultScrollingDuration = DEFAULT_SWIPING_TO_REFRESH_TO_DEFAULT_SCROLLING_DURATION;

	/**
	 * <b>ATTRIBUTE:</b> Scrolling duration status release to refresh ->
	 * refreshing
	 */
	private int mReleaseToRefreshToRefreshingScrollingDuration = DEFAULT_RELEASE_TO_REFRESHING_SCROLLING_DURATION;

	/**
	 * <b>ATTRIBUTE:</b> Refresh complete delay duration
	 */
	private int mRefreshCompleteDelayDuration = DEFAULT_REFRESH_COMPLETE_DELAY_DURATION;

	/**
	 * <b>ATTRIBUTE:</b> Scrolling duration status refresh complete -> default
	...
	/**
	 * <b>ATTRIBUTE:</b> Scrolling duration swiping to load more -> default
	 */
	private int mSwipingToLoadMoreToDefaultScrollingDuration = DEFAULT_SWIPING_TO_LOAD_MORE_TO_DEFAULT_SCROLLING_DURATION;

	/**
	 * <b>ATTRIBUTE:</b> Scrolling duration status default -> loading more,
	 * mainly for auto load more {@link #setLoadingMore(boolean)} true
	 */
		...
		try {
			...
			for (int i = 0; i < N; i++) {
				...
				if (attr == R.styleable.SwipeToLoadLayout_refresh_enabled) {
					...
					setReleaseToRefreshingScrollingDuration(a.getInt(attr,
					...
					setRefreshCompleteToDefaultScrollingDuration(a.getInt(attr,
					...
					setDefaultToRefreshingScrollingDuration(a.getInt(attr,
					...
					setReleaseToLoadingMoreScrollingDuration(a.getInt(attr,
					...
					setLoadMoreCompleteDelayDuration(a
							.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION));
					...
					setDefaultToLoadingMoreScrollingDuration(a.getInt(attr,
	...
	/**
	 * TODO add gravity LayoutParams of RefreshLoadMoreLayout
	 */
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			// swipeToRefresh -> finger up -> finger down if the status is still
			// swipeToRefresh
			// in onInterceptTouchEvent ACTION_DOWN event will stop the scroller
			// if the event pass to the child view while ACTION_MOVE(condition
			// is false)
			// in onInterceptTouchEvent ACTION_MOVE the ACTION_UP or
			// ACTION_CANCEL will not be
			// passed to onInterceptTouchEvent and onTouchEvent. Instead It will
			// be passed to
			// child view's onTouchEvent. So we must deal this situation in
			// dispatchTouchEvent
			onActivePointerUp();
			break;
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (STATUS.isSwipingToRefresh(mStatus) || STATUS.isSwipingToLoadMore(mStatus)
				...
				// abort autoScrolling, not trigger the method
				// #autoScrollFinished()
			...
			// 1. children consumed:
			// if at least one of children onTouchEvent() ACTION_DOWN return
			// true.
			// ACTION_DOWN event will not return to
			// SwipeToLoadLayout#onTouchEvent().
			// but the others action can be handled by
			// SwipeToLoadLayout#onInterceptTouchEvent()

			// 2. children not consumed:
			// if children onTouchEvent() ACTION_DOWN return false.
			// ACTION_DOWN event will return to SwipeToLoadLayout's
			// onTouchEvent().
			// SwipeToLoadLayout#onTouchEvent() ACTION_DOWN return true to
			// consume the ACTION_DOWN event.

			// anyway: handle action down in onInterceptTouchEvent() to init is
			// an good option
			break;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			// take over the ACTION_MOVE event from
			// SwipeToLoadLayout#onInterceptTouchEvent()
	...
	/**
	 * set refresh header view, the view must at lease be an implement of
	 * {@code SwipeRefreshTrigger}. the view can also implement
	 * {@code SwipeTrigger} for more extension functions
	 *
	 * @param view
	 */
	...
	/**
	 * set load more footer view, the view must at least be an implement of
	 * SwipeLoadTrigger the view can also implement {@code SwipeTrigger} for
	 ...
	 *
	 * @param dragRatio
	 *            default value is {@link #DEFAULT_DRAG_RATIO}
	 */
	...
	/**
	 * set the value of {@link #mRefreshTriggerOffset}. Default value is the
	 * refresh header view height {@link #mHeaderHeight}
	 * <p/>
	...
	/**
	 * set the value of {@link #mLoadMoreTriggerOffset}. Default value is the
	 * load more footer view height {@link #mFooterHeight}
	 * <p/>
	 ...
	 * If the offset you set is 0(default value) or smaller than
	 * {@link #mRefreshTriggerOffset} there no final offset
	 *
	 * @param offset
	 */
	 ...
	 * If the offset you set is 0(default value) or smaller than
	 * {@link #mLoadMoreTriggerOffset}, there no final offset
	 *
	 * @param offset
	 */
	...
	/**
	 * set {@link #mReleaseToRefreshToRefreshingScrollingDuration} in
	 * milliseconds
	 *
	 * @param duration
	 */
	...
	/**
	 * set {@link #mReleaseToLoadMoreToLoadingMoreScrollingDuration} in
	 * milliseconds
	 *
	 * @param duration
	 */
	...
	/**
	 * copy from
	 * {@link android.support.v4.widget.SwipeRefreshLayout#canChildScrollUp()}
	 *
	...
	protected boolean canChildScrollDown() {
		if (android.os.Build.VERSION.SDK_INT < 14) {
			if (mTargetView instanceof AbsListView) {
				final AbsListView absListView = (AbsListView) mTargetView;
				return absListView.getChildCount() > 0
						&& (absListView.getLastVisiblePosition() < absListView.getChildCount() - 1 || absListView
	...
	private void layoutChildren() {
		...
		if (mFooterView != null) {
			...
			case STYLE.SCALE:
				// scale
				footerBottom = height - paddingBottom - lp.bottomMargin + mFooterHeight / 2 + mFooterOffset
						/ 2;
				break;
			default:
				// classic
	...
	private void fingerScroll(final float yDiff) {
		...
		// forbidden fling (targetOffset>0 -> targetOffset=0 ->targetOffset<0 ->
		// default status)
		// or (targetOffset<0 -> targetOffset=0 ->targetOffset>0 -> default
		// status)
		// I am so smart :)
	...
	/**
	 * Process the scrolling(auto or physical) and append the diff values to
	 * mTargetOffset I think it's the most busy and core method. :) a ha ha ha
	 * ha...
	 *
	 * @param yScrolled
	 */
	...
	private class AutoScroller implements Runnable {
		...
		/**
		 * The param yScrolled here isn't final pos of y. It's just like the
		 * yScrolled param in the {@link #updateScroll(float yScrolled)}
		 *
		 * @param yScrolled
		 * @param duration
		 */
	...
	/**
	 * an inner util class. enum of status
	 */
