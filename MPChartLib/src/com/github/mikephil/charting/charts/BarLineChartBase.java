...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>>
	...
	protected void calcMinMax() {
		...
		if (mAutoScaleMinMaxEnabled)
			mData.calcMinMax(getLowestVisibleXIndex(),
		...
		if (mAxisLeft.isStartAtZeroEnabled()) {
			if (minLeft < 0.f && maxLeft < 0.f) {
				// If the values are all negative, let's stay in the negative
				// zone
				mAxisLeft.mAxisMinimum = Math.min(0.f, !Float.isNaN(mAxisLeft
						.getAxisMinValue()) ? mAxisLeft.getAxisMinValue()
				...
				mAxisLeft.mAxisMaximum = Math.max(0.f, !Float.isNaN(mAxisLeft
						.getAxisMaxValue()) ? mAxisLeft.getAxisMaxValue()
						: (maxLeft + topSpaceLeft));
			} else {
				// Stick the minimum to 0.0 or less, and maximum to 0.0 or more
				// (startAtZero for negative/positive at the same time)
				mAxisLeft.mAxisMinimum = Math.min(0.f, !Float.isNaN(mAxisLeft
						.getAxisMinValue()) ? mAxisLeft.getAxisMinValue()
						: (minLeft - bottomSpaceLeft));
				mAxisLeft.mAxisMaximum = Math.max(0.f, !Float.isNaN(mAxisLeft
		...
		if (mAxisRight.isStartAtZeroEnabled()) {
			if (minRight < 0.f && maxRight < 0.f) {
				// If the values are all negative, let's stay in the negative
				// zone
				mAxisRight.mAxisMinimum = Math.min(0.f, !Float.isNaN(mAxisRight
						.getAxisMinValue()) ? mAxisRight.getAxisMinValue()
						: (minRight - bottomSpaceRight));
				mAxisRight.mAxisMaximum = 0.f;
			} else if (minRight >= 0.f) {
				// We have positive values only, stay in the positive zone
				mAxisRight.mAxisMinimum = 0.f;
				mAxisRight.mAxisMaximum = Math.max(0.f, !Float.isNaN(mAxisRight
						.getAxisMaxValue()) ? mAxisRight.getAxisMaxValue()
			...
			mAxisRight.mAxisMinimum = !Float
					.isNaN(mAxisRight.getAxisMinValue()) ? mAxisRight
					.getAxisMinValue() : (minRight - bottomSpaceRight);
			mAxisRight.mAxisMaximum = !Float
					.isNaN(mAxisRight.getAxisMaxValue()) ? mAxisRight
	...
	protected float[] getMarkerPosition(Entry e, Highlight highlight) {
		...
		if (this instanceof BarChart) {
			...
			if (this instanceof HorizontalBarChart) {
				...
				float y = i + i * (setCount - 1) + dataSetIndex + space * i
						+ space / 2f;
				...
				float x = i + i * (setCount - 1) + dataSetIndex + space * i
						+ space / 2f;
	...
	/**
	 * Centers the viewport to the specified y-value on the y-axis. This also
	 ...
	 * on the y-axis. This also refreshes the chart by calling invalidate().
	 * 
	 * @param xIndex
	 * @param yValue
	 * @param axis
	 ...
	 * x-index and y-value. This also refreshes the chart by calling
	 * invalidate().
	 *
	 * @param xIndex
	 * @param yValue
	 ...
	 * @return
	 */
	public BarLineScatterCandleBubbleDataSet<? extends Entry> getDataSetByTouchPoint(
			float x, float y) {
		Highlight h = getHighlightByTouchPoint(x, y);
		if (h != null) {
	...
	/**
	 * Sets a custom axis renderer for the left axis and overwrites the existing
	 * one.
	 * 
	...
	/**
	 * Sets a custom axis renderer for the right acis and overwrites the
	 * existing one.
	 * 
	 ...
	 * 
	 * @param enabled
	 *            the y axis automatically adjusts to the min and max y values
