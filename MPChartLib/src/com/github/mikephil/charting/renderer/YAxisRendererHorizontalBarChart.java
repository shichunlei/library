...
public class YAxisRendererHorizontalBarChart extends YAxisRenderer {
	...
	public void renderAxisLabels(Canvas c) {
		...
		if (dependency == AxisDependency.LEFT) {
			...
			if (labelPosition == YAxisLabelPosition.OUTSIDE_CHART) {
				yPos = mViewPortHandler.contentBottom() + textHeight
						+ baseYOffset;
			} else {
				yPos = mViewPortHandler.contentBottom() + textHeight
						+ baseYOffset;
	...
	public void renderLimitLines(Canvas c) {
		...
		for (int i = 0; i < limitLines.size(); i++) {
			...
			if (label != null && !label.equals("")) {
				...
				final LimitLine.LimitLabelPosition position = l
						.getLabelPosition();

				if (position == LimitLine.LimitLabelPosition.RIGHT_TOP) {

					final float labelLineHeight = Utils.calcTextHeight(
							mLimitLinePaint, label);
					mLimitLinePaint.setTextAlign(Align.LEFT);
					c.drawText(label, pts[0] + xOffset,
							mViewPortHandler.contentTop() + yOffset
									+ labelLineHeight, mLimitLinePaint);
