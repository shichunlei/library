...
public class IdentifyNumPage extends FakeActivity implements OnClickListener,
	...
	public void onCreate() {
		...
		if (resId > 0) {
			...
			tvUnreceiveIdentify = (TextView) activity
					.findViewById(R.id.tv_unreceive_identify);

			String unReceive = String.format(
					getContext().getString(R.string.smssdk_receive_msg), time);
		...
		runOnUIThread(new Runnable() {
			public void run() {
				...
				if (time == 0) {
					...
					String unReceive = String.format(
							getContext().getString(R.string.smssdk_receive_msg),
							time);
