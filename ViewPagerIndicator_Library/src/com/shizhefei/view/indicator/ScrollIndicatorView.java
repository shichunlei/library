...
public class ScrollIndicatorView extends HorizontalScrollView implements Indicator {
	...
	public ScrollIndicatorView(Context context, AttributeSet attrs) {
		...
		fixedIndicatorView
				.setOnItemSelectListener(proxyOnItemSelectListener = new ProxyOnItemSelectListener());
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (unScrollPosition != -1) {
			...
			if (tabView != null) {
				final int scrollPos = tabView.getLeft() - (getMeasuredWidth() - tabView.getMeasuredWidth())
						/ 2;
				if (scrollPos >= 0) {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		if (isPinnedTabView) {
			float x = ev.getX();
			float y = ev.getY();
			if (pinnedTabView != null && y >= pinnedTabView.getTop() && y <= pinnedTabView.getBottom()
					&& x > pinnedTabView.getLeft() && x < pinnedTabView.getRight()) {
				if (ev.getAction() == MotionEvent.ACTION_DOWN) {
					mActionDownHappened = true;
				} else if (ev.getAction() == MotionEvent.ACTION_UP) {
					if (mActionDownHappened) {
						pinnedTabView.performClick();
						invalidate(new Rect(0, 0, pinnedTabView.getMeasuredWidth(),
								pinnedTabView.getMeasuredHeight()));
	...
	public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
		...
		float offset = (tabView.getWidth() + (tabView2 == null ? tabView.getWidth() : tabView2.getWidth()))
				/ 2 * positionOffset;
	...
	private int dipToPix(float dip) {
		int size = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dip, getResources()
				.getDisplayMetrics());
		return size;
	}
